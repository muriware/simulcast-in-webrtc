...
function show(stream, isRemote) {
  ...
  document
    .getElementById(isRemote ? 'remotes' : 'local')
  ...
  const bitrateGraph = new TimelineGraphView(
    id + 'Container',
    id + 'BitrateCanvas'
  );
  ...
  const framerateGraph = new TimelineGraphView(
    id + 'Container',
    id + 'FramerateCanvas'
  );
...
async function draw(pc1, pc2) {
  ...
  res1.forEach((report) => {
    ...
    if (lastSendResult && lastSendResult.get(report.id)) {
      ...
      // calculate bitrate
      const bitrate =
        (8000 * (bytes - lastSendResult.get(report.id).bytesSent)) /
      ...
      //  calculate framerate.
      const framerate =
        (1000 * (frames - lastSendResult.get(report.id).framesEncoded)) /
        (now - lastSendResult.get(report.id).timestamp);
  ...
  bitrateGraphs[graphName].setDataSeries(
    Array.from(bitrateSeries[graphName].values())
  );
  bitrateGraphs[graphName].updateEndDate();

  framerateGraphs[graphName].setDataSeries(
    Array.from(framerateSeries[graphName].values())
  );
  ...
  res2.forEach((report) => {
    ...
    if (lastRecvResult && lastRecvResult.get(report.id)) {
      let graphName = res2.get(report.trackId).trackIdentifier;
      ...
      // calculate bitrate
      const bitrate =
        (8000 * (bytes - lastRecvResult.get(report.id).bytesReceived)) /
      ...
      //  calculate framerate.
      const framerate =
        (1000 * (frames - lastRecvResult.get(report.id).framesDecoded)) /
        (now - lastRecvResult.get(report.id).timestamp);
